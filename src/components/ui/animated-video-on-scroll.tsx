...
// I added HeroImage to support next/image for projects!
export const HeroImage = React.forwardRef<
  HTMLImageElement,
  Omit<HTMLMotionProps<"img">, "src"> & { src: string; alt: string; fill?: boolean }
>(({ style, className, transition, src, alt, fill, ...props }, ref) => {
  const { scrollYProgress } = useContainerScrollContext()
  const scale = useTransform(scrollYProgress, [0, 0.8], [0.7, 1])
